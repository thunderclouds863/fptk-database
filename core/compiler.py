...
def compile_db_sourcing(db: Session, df: pd.DataFrame, user_id: int, cycle_id: int,
    ...
    for idx, row in df.iterrows():
        ...
        # ============================================================
        # PERBAIKAN UTAMA: SOURCING DATE
        # ============================================================
        sourcing_date_raw = row.get('sourcing_date')
        sourcing_date = safe_date(sourcing_date_raw)
        
        # Jika sourcing_date kosong atau None, gunakan tanggal hari ini
        if not sourcing_date:
            ...
            warnings.append({
                ...
                "warning": True,
                "error": f"Sourcing Date kosong, otomatis diisi dengan {sourcing_date.strftime('%d/%m/%Y')}"
            })
        ...
        if not kode_unik or not nama:
            warnings.append({
                "row": row_num,
                "field": "Data",
                "value": f"kode_unik={kode_unik}, nama={nama}",
                "warning": True,
                "error": "Kode Unik atau Nama kosong, row di-skip"
            })
            continue
        ...
        try:
            ...
            # ============================================================
            # DATE FIELDS (pipeline stages)
            # ============================================================
            tanggal_sourcing_freelance = safe_date(row.get('tanggal_sourcing_freelance'))
            tanggal_sourcing = safe_date(row.get('tanggal_sourcing'))
            tanggal_shortlist_cv = safe_date(row.get('tanggal_shortlist_cv'))
            tanggal_psikotes = safe_date(row.get('tanggal_psikotes'))
            tanggal_hr_interview = safe_date(row.get('tanggal_hr_interview'))
            tanggal_user_interview = safe_date(row.get('tanggal_user_interview'))
            tanggal_offering = safe_date(row.get('tanggal_offering'))
            tanggal_day1 = safe_date(row.get('tanggal_day1'))
            ...
            tanggal_technical_test = safe_date(row.get('tanggal_technical_test'))
            ...
            tanggal_market_visit = safe_date(row.get('tanggal_market_visit'))
            ...
            tanggal_panel_interview = safe_date(row.get('tanggal_panel_interview'))
            ...
            tanggal_reference_check = safe_date(row.get('tanggal_reference_check'))
