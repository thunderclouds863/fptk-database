...
if "is_logged_in" not in st.session_state:
    st.session_state.is_logged_in = session_mgr.is_logged_in
...
# ============================================================
# SESSION RESTORE - PRIORITAS SESSION_STATE
# ============================================================

# Jika session_state ada user_id tapi session_manager kosong → restore
...
# Jika session_manager ada tapi session_state kosong → restore
...
# ============================================================
# VALIDASI SESSION (Cek user masih ada di DB)
# ============================================================
...
# ============================================================
# INIT DEFAULT USERS & MASTER DROPDOWN
# ============================================================
...
if not st.session_state.user_id:
    ...
    @media (max-width: 768px) {
        ...
        .cimory-logo-container { width: 100vw !important; max-width: 100vw !important; left: 50% !important; transform: translateX(-50%) !important; margin-top: 10px !important; margin-bottom: 35px !important; }
        .cimory-logo { width: 220px !important; max-width: 220px !important; }
        div[data-testid="stForm"] { width: auto !important; max-width: calc(100vw - 30px) !important; padding: 30px 22px 28px 22px !important; border-radius: 17px !important; }
        .login-title { font-size: 32px; gap: 10px; }
        .login-icon { font-size: 26px !important; width: 34px; height: 34px; }
        div[data-baseweb="input"] { height: 56px !important; }
        div[data-baseweb="input"] input { height: 54px !important; font-size: 15px !important; }
        div[data-testid="stFormSubmitButton"] button { height: 56px !important; }
    }
    </style>
    ...
    # LOGO
    if logo_base64:
        st.markdown(f'<div class="cimory-logo-container"><img src="data:image/png;base64,{logo_base64}" class="cimory-logo" alt="Cimory Logo"></div>', unsafe_allow_html=True)
    ...
    if cache_funcs:
        ...
        with col2:
            if st.button("🗑️ Clear Cache", use_container_width=True):
                cache_funcs['load_fptk_data'].clear()
                cache_funcs['load_sourcing_data'].clear()
                cache_funcs['calculate_metrics'].clear()
                cache_funcs['get_upload_cycle_progress'].clear()
...
# ============================================================
# PAGE RENDER FUNCTION WITH SPINNER
# ============================================================

def render_page(page_name: str, module_name: str, function_name: str = "show_dashboard"):
    """Render page dengan loading spinner dan error handling"""
    try:
        with st.spinner(f"⏳ Memuat {page_name}..."):
            module = importlib.import_module(module_name)
            func = getattr(module, function_name)
            func()
    except ModuleNotFoundError as e:
        st.error(f"❌ File {module_name}.py tidak ditemukan! Error: {str(e)}")
        st.info("💡 Pastikan semua file page ada di folder `pages/`")
    except AttributeError as e:
        st.error(f"❌ Fungsi {function_name} tidak ditemukan di {module_name}.py! Error: {str(e)}")
    except Exception as e:
        st.error(f"❌ Error memuat {page_name}: {str(e)}")
...
if page == "dashboard":
    render_page("Dashboard", "pages.dashboard", "show_dashboard")

elif page == "upload_compile":
    render_page("Upload & Compile", "pages.02_upload_compile", "show_upload_compile")

elif page == "fptk_view":
    render_page("FPTK View", "pages.03_fptk_view", "show_fptk_view")

elif page == "sourcing_view":
    render_page("Sourcing View", "pages.04_sourcing_view", "show_sourcing_view")

elif page == "db_kode_posisi":
    render_page("DB Kode Posisi", "pages.05_db_kode_posisi", "show_db_kode_posisi")

elif page == "upload_cycle":
    render_page("Update Cycle", "pages.06_upload_cycle", "show_upload_cycle")

elif page == "user_management":
    render_page("User Management", "pages.07_user_management", "show_user_management")

elif page == "sourcing_input":
    render_page("Sourcing Input", "pages.09_sourcing_input", "show_sourcing_input")

elif page == "funnel_report":
    render_page("Funnel Report", "pages.funnel_report", "show_funnel_report")

elif page == "monitoring_sourcing":
    render_page("Monitoring Sourcing", "pages.monitoring_sourcing", "show_monitoring_sourcing")

elif page == "upload_evidence":
    render_page("Upload Evidence", "pages.upload_evidence", "show_upload_evidence")

elif page == "transfer_fptk":
    render_page("Transfer FPTK", "pages.transfer_fptk", "show_transfer_fptk")

else:
    st.error(f"❌ Halaman '{page}' tidak dikenal!")


# ============================================================
# EXPORT MENU (Di Bawah Konten)
# ============================================================
...
# ============================================================
# AUTO REFRESH TIMER (optional - background)
# ============================================================

# Check if 5 minutes have passed since last load
if (datetime.now() - st.session_state.last_fptk_load).seconds > 300:
    # Auto refresh cache silently (tanpa rerun)
    st.cache_data.clear()
    st.session_state.last_fptk_load = datetime.now()
    st.session_state.last_sourcing_load = datetime.now()
