                        ...
                        if st.session_state.get(f"show_reject_{req.id}", False):
                            with st.form(f"reject_form_{req.id}"):
                                reject_reason = st.text_area(
                                    "Alasan Reject *",
                                    placeholder="Contoh: FPTK masih dibutuhkan...",
                                    height=100
                                )
                    ...
                    with col2:
                        ...
                        if st.session_state.get(f"show_reject_src_{req.id}", False):
                            with st.form(f"reject_src_form_{req.id}"):
                                reject_reason = st.text_area(
                                    "Alasan Reject *",
                                    placeholder="Contoh: Data masih dibutuhkan...",
                                    height=100
                                )
                                if st.form_submit_button("Konfirmasi Reject", type="primary"):
                                    if not reject_reason or len(reject_reason.strip()) < 5:
                    ...
                    with col2:
                        ...
                        if st.button(f"✅ Approve", key=f"approve_bl_{req.id}", type="primary", use_container_width=True):
                            try:
                                ...
                                st.cache_data.clear()
                                st.success(f"✅ Kandidat {req.nama} di-blacklist!")
                                time.sleep(0.5)
                                st.rerun()
                            except Exception as e:
                                st.error(f"❌ Error: {str(e)}")
                        ...
                        if st.session_state.get(f"show_reject_bl_{req.id}", False):
                            with st.form(f"reject_bl_form_{req.id}"):
                                reject_reason = st.text_area(
                                    "Alasan Reject *",
                                    placeholder="Contoh: Kandidat masih bisa dipertimbangkan...",
                                    height=100
                                )
                                if st.form_submit_button("Konfirmasi Reject", type="primary"):
                                    if not reject_reason or len(reject_reason.strip()) < 5:
                    ...
                    with col2:
                        ...
                        if st.session_state.get(f"show_reject_unbl_{req.id}", False):
                            with st.form(f"reject_unbl_form_{req.id}"):
                                reject_reason = st.text_area(
                                    "Alasan Reject *",
                                    placeholder="Contoh: Kandidat masih bermasalah...",
                                    height=100
                                )
                                if st.form_submit_button("Konfirmasi Reject", type="primary"):
                                    if not reject_reason or len(reject_reason.strip()) < 5:
