...
def get_filter_options():
    ...
    Return tuple (pic_options, bu_options, dir_options) dengan "Semua" di depan.
    """
    try:
...
def load_fptk_data(
    ...
    Bikin session DB BARU tiap query, close di finally.
    """
    db = SessionLocal()
    try:
        query = db.query(FPTK)
...
def show_dashboard():
    ...
    with st.sidebar:
        ...
        pic_filter = st.selectbox("PIC Recruiter", pic_options, key="dash_pic_filter")

        # ============================================================
        # Status (HARDCODED - enum tetap)
        # ============================================================
        status_options = ["Semua"] + filter_opts.get("status_options", ["OP", "Closed", "Cancel"])
        status_filter = st.selectbox("Status", status_options, key="dash_status_filter")

        # ============================================================
        # Business Unit (DINAMIS dari master_dropdown)
        # ============================================================
        bu_options = ["Semua"] + filter_opts.get("bu_options", [])
        bu_filter = st.selectbox("Business Unit", bu_options, key="dash_bu_filter")

        # ============================================================
        # Direktorat (DINAMIS dari master_dropdown)
        # ============================================================
        dir_options = ["Semua"] + filter_opts.get("direktorat_options", [])
        dir_filter = st.selectbox("Direktorat", dir_options, key="dash_dir_filter")

        # ============================================================
        # Filter Kategorisasi (DINAMIS dari master_dropdown)
        # ============================================================
        filter_kat_options = ["Semua"] + filter_opts.get("filter_kategorisasi_options", [])
        filter_kat = st.selectbox("Filter Kategorisasi", filter_kat_options, key="dash_kat_filter")

        st.markdown("---")

        if st.button("📥 Export CSV", use_container_width=True, key="dash_export_btn"):
            st.session_state.export_data = True

        if st.button("🔄 Refresh Filter Options", use_container_width=True, key="dash_refresh_filter_btn"):
            ...
            if 'fptk_date_real' in df and df['fptk_date_real'].notna().any():
                ...
                if len(heatmap_data) > 0:
                    fig = px.density_heatmap(
                        heatmap_data, x='day', y='month', z='count',
    ...
    if admin:
        ...
        if not df_progress.empty:
            ...
            st.progress(
                done / len(df_progress) if len(df_progress) > 0 else 0,
