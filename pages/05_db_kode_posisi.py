...
def show_db_kode_posisi():
    ...
    st.markdown("Master database posisi, lokasi, dan mapping user.")
    ...
    if not admin:
        st.warning("Hanya Admin yang bisa menambah/mengedit DB Kode Posisi.")
    else:
        with st.form("add_position_form"):
            ...
            if submitted:
                if not position:
                    ...
                    if existing:
                        st.warning(f"Position '{position}' dengan BU '{business_unit}' dan Location '{location}' sudah ada!")
                    else:
                        try:
    ...
    if total > 0 and admin:
        st.markdown("---")
        st.markdown("### Edit / Hapus Position")
        ...
        if selected_display:
            ...
            if detail:
                with st.form("edit_position_form"):
                    ...
                    with col2:
                        delete_btn = st.form_submit_button("Hapus Position", type="secondary")
                    ...
                    if delete_btn:
                        st.session_state["confirm_delete_pos"] = detail.id

                if st.session_state.get("confirm_delete_pos") == detail.id:
                    st.warning(f"Yakin ingin menghapus position '{detail.position}'?")
                    col_yes, col_no = st.columns(2)
                    with col_yes:
                        if st.button("Ya, Hapus", key="confirm_delete_position_yes", type="primary"):
                            try:
                                db.delete(detail)
                                db.commit()
                                st.session_state["confirm_delete_pos"] = None
                                st.success("Position berhasil dihapus!")
                                st.rerun()
                            except Exception as e:
                                st.error(f"Error: {str(e)}")
                                db.rollback()
                    with col_no:
                        if st.button("Batal", key="confirm_delete_position_no"):
                            st.session_state["confirm_delete_pos"] = None
                            st.rerun()
