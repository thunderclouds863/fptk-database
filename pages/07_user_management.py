...
from core.auth import get_current_user, is_admin, create_user, reset_password
import pandas as pd
...
def show_user_management():
    ...
    # ============================================================
    # DAFTAR USER (DENGAN TOMBOL HAPUS DI SEBELAH CREATED AT)
    # ============================================================
    st.subheader("📋 Daftar User")
    users = db.query(User).order_by(User.username).all()
    
    if users:
        # Buat data dengan kolom Aksi (placeholder)
        data = []
        for u in users:
            # Tentukan tombol hapus (hanya untuk user bukan admin yang sedang login)
            ...
            data.append({
                ...
                "Aksi": "🗑️ Hapus" if can_delete else "-"
            })
        df = pd.DataFrame(data)
        
        # Tampilkan dengan data_editor (read-only kecuali aksi)
        edited_df = st.data_editor(
            df,
            use_container_width=True,
            height=400,
            disabled=["ID", "Username", "Role", "Display Name", "Last Login", "Created"],
            column_config={
        ...
        # Proses hapus dari data_editor
        # Cek apakah ada perubahan di kolom Aksi (tapi data_editor ga trigger event)
        # Kita pakai tombol di bawah sebagai alternatif
        
        # ============================================================
        # TOMBOL HAPUS DI BAWAH TABEL (TAPI DENGAN DROPDOWN + KONFIRMASI)
        # ============================================================
        st.markdown("---")
        st.subheader("🗑️ Hapus User dari Daftar")
        
        # Dropdown pilih user yang mau dihapus
        delete_options = {}
        for u in users:
            if u.id != user.id:  # Jangan hapus diri sendiri
                delete_options[f"{u.username} ({u.role}) - Created: {u.created_at.strftime('%d/%m/%Y') if u.created_at else '-'}"] = u.id
    ...
    if user_options:
        ...
        with col1:
            selected_user = st.selectbox("Pilih User", list(user_options.keys()), key="reset_user_select")
        with col2:
            if selected_user:
