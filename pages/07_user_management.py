...
from core.auth import get_current_user, is_admin, is_it, create_user, reset_password
...
def show_user_management():
    ...
    with st.form("add_user"):
        ...
        with col3:
            new_role = st.selectbox("Role", ["user", "admin", "it"])
    ...
    user_options = {f"{u.username} ({u.display_name or u.username})": u.id for u in users if u.role in ["user", "it"]}
    if user_options:
    ...
    st.markdown("---")
    st.subheader("✏️ Edit Role User")
    
    role_options = ["user", "admin", "it"]
    edit_users = {f"{u.username} ({u.display_name or u.username})": u for u in users}
    selected_edit = st.selectbox("Pilih User untuk edit role", list(edit_users.keys()) if edit_users else [])
    
    if selected_edit:
        target_user = edit_users[selected_edit]
        new_role = st.selectbox("Role Baru", role_options, index=role_options.index(target_user.role) if target_user.role in role_options else 0)
        
        if st.button("Update Role"):
            target_user.role = new_role
            db.commit()
            st.success(f"✅ Role user '{target_user.username}' diubah menjadi '{new_role}'")
            st.rerun()
